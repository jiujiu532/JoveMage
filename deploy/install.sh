...
REPO_OWNER="${REPO_OWNER:-jiujiu532}"
REPO_NAME="${REPO_NAME:-JoveMage}"
BRANCH="${BRANCH:-main}"
INSTALL_DIR="${INSTALL_DIR:-/opt/jovemage}"
...
usage() {
  ...
  curl -fsSL https://raw.githubusercontent.com/jiujiu532/JoveMage/main/deploy/install.sh | sudo bash
EOF
  ...
  INSTALL_LANG=zh|en
  CHATGPT2API_IMAGE=ghcr.io/jiujiu532/jovemage:latest
...
EOF
  ...
  --port 3000
  --thread-tokens 80
  --install-dir /opt/jovemage
  --branch main
  --auth-key <replace-with-a-long-random-secret>
...
text() {
  ...
  if is_en; then
    case "${key}" in
      usage_title) printf 'JoveMage installer' ;;
      ...
      info_start_app) printf 'Starting JoveMage on' ;;
      ...
      done_ready) printf 'JoveMage is ready' ;;
  ...
  case "${key}" in
    ...
    usage_title) printf 'JoveMage 安装脚本' ;;
    ...
    info_start_app) printf '正在启动 JoveMage' ;;
    ...
    done_ready) printf 'JoveMage 已就绪' ;;
