...
Python ≥ 3.13，依赖用 **uv** 管理（`uv.lock`，默认走阿里云 PyPI 镜像）。
...
# 前端（web-vue/ 下执行）
npm install
npm run dev      # Vite 开发服务器，/api /v1 /auth 等代理到 VITE_DEV_API_TARGET（默认 localhost:8000）
npm run build    # clean:dist && tsc && vite build → web-vue/dist
```

无 lint / formatter 配置，也没有 pytest；测试一律用标准库 `unittest`。CI 只有 `.github/workflows/docker-publish.yml`（构建并推 GHCR 镜像），不跑测试。
...
**禁止在本地做 Docker 构建或运行**（见 AGENTS.md）。涉及容器的改动只做静态检查，说明未做本地验证。
...
3. **`LoggedCall.run()`** `services/log_service.py` — 底层 curl_cffi 是**同步**的，所有 handler 都必须丢进 `run_in_threadpool`；这一层统一做异常 → OpenAI/Anthropic 错误体转换、SSE 包装、各阶段耗时上报。写新接口时把业务逻辑交给它，不要自己拼 `StreamingResponse`。
...
6. **协议转换** `services/protocol/conversation.py` — ChatGPT 内部事件流 → 标准 `chat.completion.chunk` 或 Anthropic events。
...
- **Token 续期** `_request_access_token_refresh()` 必须用固定 `client_id=app_2SKx67EdpoN0G6j64rFvigXD`，改错会导致全量续期失败。除 access_token 按提前量刷新外，refresh_token 每 3 天主动保活防吊销。
...
运行时读仓库根的 `config.json`（不入库），`config.example.yaml` 只是带注释的示例。`auth-key` 缺失或仍是占位值时**服务拒绝启动**（`services/config.py`）。环境变量 `CHATGPT2API_AUTH_KEY` 优先级高于 `config.json`。

### 前端

Vue 3.5 + TS + Vite 7 + Tailwind 3 + Pinia + vue-router（**hash 模式**），UI 用私有包 `nanocat-ui`，echarts 走 `public/vendor/` 静态引入而非 npm 依赖。

- 认证是把密码原样存 localStorage（key `chatgpt2api.adminKey`）当 Bearer token 用。
...
| 路线 | 架构 | 说明 |
|---|---|---|
| `install.sh`（主推，2.7.0 重写） | 应用 → SOCKS5 → WARP，**无 Privoxy**，1-6 实例 | 生产部署脚本，`proxy_pool` 形如 `socks5h://warp-N:1080` |
...
三个 compose 文件：`docker-compose.yml`（拉 GHCR 镜像，生产）、`.local.yml`（本地构建，sqlite，8000:80）、`.warp.yml`（五服务 WARP 全栈）。

## 需要留意的现状

- `services/openai_backend_api.py`（~3400 行）、`services/protocol/conversation.py`（~2500 行）、`services/register/mail_provider.py`（~2100 行）三个巨型文件占后端约三分之一，改动前先定位再动，不要整体重排。
- `.trellis/spec/backend/` 下全是 "To fill" 模板，没有已落地的编码规范；不要把模板内容当成项目约定引用。
- 根目录 `参考/chatgpt2api-main` 是上游二代原项目，仅供对照，**不是本仓库代码**。
