...
MAX_DX_CHARS: Final = 1_048_576
MAX_DECODED_BYTES: Final = 524_288
MAX_VM_STEPS: Final = 20_000
MAX_VM_NESTING: Final = 32


class _TurnstileVMResourceError(RuntimeError):
    pass
...
def solve_turnstile_token(dx: str, p: str) -> Optional[str]:
    ...
    def run_queue(queue: list[Any] | None = None) -> None:
        nonlocal vm_nesting, vm_steps
        if vm_nesting >= MAX_VM_NESTING:
            raise _TurnstileVMResourceError
        previous_queue = process_map.get(9)
        vm_nesting += 1
        ...
        try:
            while isinstance(process_map.get(9), list) and process_map[9]:
                ...
                try:
                    fn(*token[1:])
                except _TurnstileVMResourceError:
                    raise
                except Exception:
                    continue
        finally:
            process_map[9] = previous_queue
            vm_nesting -= 1
