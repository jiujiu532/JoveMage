...
export function normalizeSettings(raw: RawSettings | null | undefined): Settings {
  ...
  const normalized = {
    ...
    ai_review: {
      ...
      has_api_key: boolValue(aiReview.has_api_key, false),
    ...
    image_storage: {
      ...
      has_webdav_password: boolValue(imageStorage.has_webdav_password, false),
      webdav_root_path: cleanString(imageStorage.webdav_root_path) || 'jovemage/images',
    ...
    backup: {
      ...
      has_secret_access_key: boolValue(backup.has_secret_access_key, false),
      ...
      has_passphrase: boolValue(backup.has_passphrase, false),
      include: {
    ...
    firefly_poll_interval_sec: Math.trunc(numberValue(source.firefly_poll_interval_sec, 3, 1)),
...
function toBackendSettings(settings: Settings): RawSettings {
  ...
  const payload: RawSettings = {
    ...
    firefly_poll_interval_sec: Math.trunc(numberValue(normalized.firefly_poll_interval_sec, 3, 1)),
