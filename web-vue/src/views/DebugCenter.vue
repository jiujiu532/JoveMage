...
const searchPrompt = ref('帮我搜索 JoveMage 相关项目')
...
const chatInput = ref('你好，先记住我的项目叫 JoveMage。')
...
name: jovemage-search
description: 当用户需要联网搜索、查询最新信息、核实事实或需要来源链接时，调用本地 JoveMage 搜索接口。
---
...
name: jovemage-search
description: Use when current web search is needed through this JoveMage server. Call the configured HTTP search endpoint with a prompt and return the answer with source URLs.
---
...
watch(activeTab, (tab) => {
  if (tab === 'ppt') {
    editablePrompt.value = '生成一个 JoveMage 项目架构说明 PPT，包含后端链路、前端控制台、账号池、日志和图片链路。'
