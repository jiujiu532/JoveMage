...
TASK_DETAIL_KEYS = (
    "error_code",
    # 以下 7 个 failure_* / status_code / error_type 字段来自
    # services.image_failure.ImageFailure.diagnostic_fields()，用于落库结构化失败分类
    "failure_scope",
    "failure_capability",
    "failure_retryable",
...
def _classify_task_failure(exc: Exception) -> dict[str, Any]:
    """对图片任务异常做结构化失败分类，补充 error_code / failure_* 诊断字段。

    不改写面向用户的错误文案（沿用 str(exc) 原文），只在诊断字段里落库
    services.image_failure 的结构化分类结果，供后续切号/统计/续轮询判断使用。
    """
...
class ImageTaskService:
    ...
    def _run_task(
        ...
        try:
            ...
            error_details = _classify_task_failure(exc)
            duration_ms = int((time.time() - started) * 1000)
    ...
    def _recover_unfinished_locked(self) -> bool:
        ...
        for task in self._tasks.values():
            if task.get("status") in UNFINISHED_STATUSES:
                raw_error = "服务已重启，未完成的图片任务已中断"
                # 沿用本地既有的中文提示文案，仅额外补充结构化失败分类（task_interrupted）
                # 供落库/统计使用，不采用对方版本里改写用户可见文案的做法。
                interrupted = ImageFailureError(
                    raw_error,
                    failure=image_failure("task_interrupted", raw_detail=raw_error),
                )
                details = _classify_task_failure(interrupted)
                task["status"] = TASK_STATUS_ERROR
                task["error"] = raw_error
                for detail_key, value in _task_detail_fields(details).items():
                    if value in (None, ""):
                        task.pop(detail_key, None)
                        continue
                    task[detail_key] = value
                task["updated_at"] = _now_iso()
                changed = True
        return changed
    ...
    def resume_poll(
        ...
        with self._lock:
            ...
            # 轮询超时是唯一允许"续轮询"的失败类型：用落库的结构化 error_code
            # （而非在错误文案里模糊匹配"超时"字样）判断，避免误判/漏判。
