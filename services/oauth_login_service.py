...
class OAuthLoginService:
    ...
    def _extract_code_from_callback(value: str) -> tuple[str, str]:
        ...
        if raw.startswith("http://") or raw.startswith("https://"):
            try:
                callback_url = validate_continuation_url(raw)
                parsed = parse_qs(urlparse(callback_url).query)
            except Exception as exc:
                raise OAuthLoginError(
                    f"无法解析 callback URL: {redact_auth_diagnostic(exc)}"
                ) from exc
            ...
            if not code:
                ...
                raise OAuthLoginError(
                    redact_auth_diagnostic(err or "callback URL 中没有 code 参数")
                )
            return code, state
        # 用户可能直接粘了 code 字符串
        return raw, ""
    ...
    def _exchange_code(code: str, code_verifier: str, redirect_uri: str) -> dict[str, str]:
        ...
        try:
            ...
            raise OAuthLoginError(
                f"换 token 网络异常: {redact_auth_diagnostic(exc)}"
            ) from exc
        finally:
            session.close()
        ...
        if response.status_code != 200 or not isinstance(data, dict) or not data.get("access_token"):
            ...
            if not detail:
                try:
                    detail = redact_auth_diagnostic(response.text, 300)
                except Exception:
                    detail = ""
            detail = redact_auth_diagnostic(detail, 300)
            raw_body = redact_auth_diagnostic(getattr(response, "text", ""), 500)
            print(
