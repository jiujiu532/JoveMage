...
class RealtimeMonitorService:
    ...
    def finish(self, detail: dict[str, Any]) -> None:
        ...
        if is_text_review_failure_code(failure_code):
            status = "text_review"
        if status == "success":
            stage, stage_label = "completed", STAGE_LABELS["completed"]
        elif status == "text_review":
            stage, stage_label = "text_review", "文本"
        else:
            stage, stage_label = "failed", STAGE_LABELS["failed"]
        with self._lock:
    ...
    def _summary(self, active: list[dict[str, Any]], completed: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        text_review = sum(1 for item in completed if str(item.get("status") or "").lower() == "text_review")
