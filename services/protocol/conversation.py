...
def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    ...
    if is_firefly_model(request.model):
        from services.protocol.firefly_orchestration import _stream_firefly_image_outputs
...
# ---------------------------------------------------------------------------
# Firefly orchestration re-exports (compat for tests / external importers).
# Symbols live in firefly_orchestration; resolved lazily to avoid cycles.
# ---------------------------------------------------------------------------
_FIREFLY_REEXPORTS = frozenset({
    "_MAX_FIREFLY_REFERENCE_IMAGES",
    "_BASE64_CHARSET_RE",
    "_guess_image_mime",
    "_looks_like_base64",
    "_decode_firefly_image_entry",
    "_normalize_firefly_image_inputs",
    "_coerce_firefly_image_bytes",
    "_build_firefly_image2image_payload",
    "_firefly_image_result_output",
    "_generate_single_image_firefly",
    "_generate_single_image_firefly_edit",
    "_stream_firefly_image_outputs",
    "_firefly_account_id",
    "_save_firefly_video_bytes",
    "_coerce_firefly_video_result",
...
def __getattr__(name: str) -> Any:
    if name in _FIREFLY_REEXPORTS:
        from services.protocol import firefly_orchestration as _firefly_orchestration

        return getattr(_firefly_orchestration, name)
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


def __dir__() -> list[str]:
    return sorted(set(globals()) | _FIREFLY_REEXPORTS)
