...
from utils.diagnostics import diagnostic_excerpt, redact_auth_diagnostic
from utils.log import logger
from utils.pow import build_legacy_requirements_token, build_proof_token, parse_pow_resources
from utils.turnstile import solve_turnstile_token

# 说明（异常体系替换，见 .trellis/tasks/07-26-sync-upstream-271）：
# InvalidAccessTokenError / ImagePollTimeoutError 直接复用 services/image_failure.py
# 中已有的结构化异常类（本地语义与其 auth_invalid / image_poll_timeout 完全一致）。
# ImageContentPolicyError / ImageTextReplyError 是本仓库独有的失败类型，对方分叉里
# 没有对应类，因此改为继承 image_failure.ImageFailureError 并各自绑定结构化失败码，
# 使其同样携带结构化的 .failure（ImageFailure 对象，可配合
# services.image_failure.public_image_error_message() 等函数使用）；
# 类名保持不变，对外行为（单个 message 位置参数、可任意 setattr 附加诊断字段）
# 与替换前完全一致，本步不改动 services/protocol/conversation.py 里的 except 分支。
...
class OpenAIBackendAPI:
    ...
    def get_user_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                {"event": "backend_user_info_bootstrap_failed", "error": redact_auth_diagnostic(exc)}
            )
