...
def _gptmail_status_payload(entry: dict, conf: dict, *, reveal_public_key: bool = False) -> dict[str, Any]:
    ...
    try:
        if key_mode == "public":
            ...
            resp = session.request("GET", f"{api_base}/api/public-key-status", headers=headers, params=params, timeout=conf["request_timeout"])
...
class GptMailProvider(BaseMailProvider):
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None):
        ...
        resp = self.session.request(method.upper(), f"{self.api_base}{path}", params=query, json=payload, timeout=self.conf["request_timeout"])
        if resp.status_code != 200:
